...
[![Version](https://img.shields.io/badge/version-v3.4-blue)](https://github.com/CasterYu/dashboard)
...
| 版本 | 链接 | 状态 | 说明 |
|:---:|:---|:---:|:---|
| **🟢 v3.4 · 最新版** | **👉 [https://casteryu.github.io/dashboard/](https://casteryu.github.io/dashboard/)** | ✅ 当前 | 六级组织树筛选（全国→大区→小区→门店→岗位→人员）+ 单选 / PK 双模式 + 全链路 7 阶段漏斗（带转化率徽章）+ **三大板块可展开按日走势折线图** + 多维下钻 + 趋势分析 + 积分榜 + 明细表 |
...
| 版本 | 文件路径 |
|:---:|:---|
| **v3.4 最新版** | `D:\projects\dashboard\index.html` |
...
- **v3.4（当前）**：**三大板块（线索入口 / 到店接待 / 结果面板）各自可「展开时间轴」**，展开后该板块的卡片墙替换为「指标多选 + 按日折线图」，默认全选、可单点增减（至少保留 1 项）、可一键「全选」恢复；三个板块互不影响，可同时展开。**收起态删除卡片右下角迷你走势**，卡片只聚焦「指标值 + 环比涨跌 + 相对上一环节占比」，界面更干净。
- **v3.3（历史）**：把右侧**全链路七阶段转化漏斗**放大，改为更醒目的横向渐变条，每阶段名称与数值置于条内，右侧以独立徽章展示「环比上一阶段转化率」，整体转化率/最大流失/线索→交付等摘要保留在漏斗底部。
...
[👉 v3.4 最新版](https://casteryu.github.io/dashboard/) · [👉 v1 历史版](https://casteryu.github.io/dashboard/legacy/v1/)
